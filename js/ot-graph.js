...
import { normKey, parseCSVSmart, stripDiacritics } from "./csv-helpers.js";
...
function addItemRow(item={cantidad:"",descripcion:"",plano:"",adjunto:""}){
  if(!itemsBox()) return;
  const row=document.createElement("div"); row.className="items-row"; row.innerHTML=`
...
function readItemsFromUI(){
  return Array.from(itemsBox()?.querySelectorAll(".items-row")||[]).map(r=>{
    const [q,d,p,a]=r.querySelectorAll("input"); return {cantidad:q.value,descripcion:d.value,plano:p.value,adjunto:a.value};
  });
}

function fillForm(data=null){
  editingIndex=data?LIST.indexOf(data):-1;
  fNum()&&(fNum().value=data?.num??"");
  fCliente()&&(fCliente().value=data?.cliente??"");
  fDepto()&&(fDepto().value=data?.depto??"");
  fEnc()&&(fEnc().value=data?.enc??"");
  fEmision()&&(fEmision().value=fmtDate(data?.emision));
  fEntrega()&&(fEntrega().value=fmtDate(data?.entrega));
  fOC()&&(fOC().value=data?.oc??"");
  fEst()&&(fEst().value=data?.est??"ABIERTA");
  fPrio()&&(fPrio().value=data?.prio??"NORMAL");
  fDesc()&&(fDesc().value=data?.desc??"");
  clearItemsUI();
...
/* ====== Import/Export/Clear ====== */
function mapRowToOT(row){
  // row keys are normalized already (via parseCSVSmart -> normKey)
  const k = Object.fromEntries(Object.entries(row).map(([kk,v])=>[normKey(kk),v]));

  // explicit common Spanish headers
  const num = k["num"] || k["n_ot"] || k["no_ot"] || k["num_ot"] || k["ot"] || k["folio"] || k["numero"] || k["__ot"];
  const cliente = k["cliente"] || k["cliente_nombre"] || k["nombre_cliente"];
  const depto = k["departamento"] || k["depto"];
  const enc = k["encargado"] || k["responsable"] || k["jefe"];
  const emision = k["fecha_emision"] || k["fecha_de_emision"] || k["f_emision"] || k["emision"] || k["fecha"];
  const entrega = k["fecha_entrega"] || k["fecha_de_entrega"] || k["f_entrega"] || k["entrega"];
  const oc = k["orden_compra"] || k["orden_de_compra"] || k["oc"] || k["ordencompra"];
  const est = k["estatus"] || k["estado"] || k["est"];
  const prio = k["prioridad"] || k["prio"];
  const desc = k["descripcion"] || k["descripción"] || k["desc"];

  return {
    num: num || "", cliente: cliente || "", depto: depto || "", enc: enc || "",
    emision: emision || "", entrega: entrega || "", oc: oc || "", est: est || "",
    prio: prio || "", desc: desc || "", items: []
  };
}

function parseCSV(text){
  const { headers, rows } = parseCSVSmart(text);
  return rows.map(mapRowToOT);
}
...
async function importFile(file){
  ...
  const recs = arr.map(r => mapRowToOT(r)).filter(x=> (x.cliente || x.desc));
...
function buildPrintHTML(rec){
  ...
  const dir="Calle 61 #232 Col. Villa Jardín, San Luis Potosí, S. L. P.";
  const tel="+52 444 829 5859";
  const mail="luis.moreno@artepisa.com";

  return `<!doctype html>
<html><head><meta charset="utf-8"><title>OT ${rec.num||""} - Artepisa</title>
<style>
  ...
  <div class="header">
    ...
    <div>
      ...
      <div class="contact">Dirección: ${dir}<br>Teléfono: ${tel} · Correo: ${mail}</div>
    </div>
  </div>
  ...
  <div class="grid">
    ...
    <div class="field"><div class="label">Estatus</div><div class="value">${rec.est?`<span>${rec.est}</span>`:"&nbsp;"}</div></div>
    <div class="field"><div class="label">Prioridad</div><div class="value">${rec.prio?`<span>${rec.prio}</span>`:"&nbsp;"}</div></div>
...
function printOT(rec){
  if(!rec || !rec.cliente){ alert("Completa al menos el CLIENTE antes de imprimir."); fCliente()?.focus(); return; }
  ...
  setTimeout(()=>{ try{ win.focus(); win.print(); }catch(_){} setTimeout(()=>frame.remove(),800); }, 300);
}
...
function mountEvents(){
  on(btnShowForm(),"click",()=>{ fillForm(null); setMode("form-only"); elCardForm()?.scrollIntoView({behavior:"smooth",block:"start"}); });
  ...
  on(btnGuardar(),"click", async ()=>{
    ...
    try{ await save(); alert("Guardado"); setMode(""); window.scrollTo({top:0,behavior:"smooth"}); }
  ...
  on(elTable(),"click",(e)=>{
    ...
    if (act === "edit"){ fillForm(LIST[i]); setMode("form-only"); elCardForm()?.scrollIntoView({behavior:"smooth"}); }
...
(async function bootstrap(){
  try{ mountEvents(); await load(); }catch(e){ console.error("Init OT falló:",e); mountEvents(); }
})();
